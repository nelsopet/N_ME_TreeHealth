...
#Read in data
spec_chem_canopy<-read.csv("output/speclib/poplar_canopy_speclib_5nm_wChem.csv") 
canopy_VIs<-read.csv("output/speclib/poplar_VIs_5nm.csv") 

#Make a list of  band names for data filtering later
band_names<-colnames(spec_chem_canopy[,32:ncol(spec_chem_canopy)])
...
Resp_names<-as.data.frame(colnames(spec_chem_canopy[,13:31]))[1:19,]
...
lapply(15:length(vars_pretty), function(x) { 
...
#    spec_chem_canopy_df %>% group_by(GenusSpecies) %>% tally
    ...
    write.csv(testing_labels,paste("output/models/predictions/poplars/",className,"_median_5nmBands_",ty,".csv",sep=""))
    })
      #Old way
...
#
  ...
  # plsFit
    ...
    #R2stat = round(R2(rf_mod_pred$predictions, testing[[className]], formula = "corr", na.rm=TRUE),2)
...
all_out<-list.files("output/models/predictions/poplars/")
...
all_preds<-lapply(1:length(all_out), function(x) 
    {read.csv(paste("output/models/predictions/poplars/",all_out[x],sep=""))
    })
all_preds<-Reduce(rbind, all_preds) 
all_preds_mean<-all_preds %>% group_by( var) %>% 
...
jpeg(paste("output/models/figs/poplars/",className,"_median_5nmBands",".jpg", sep=""), height = 500, width=550)
df_var<-all_preds[all_preds$var == className,]
#df_var_field_id<-df_var %>% ungroup() %>% dplyr::select(Field) %>% unique %>% as.data.frame()
#df_var_field_id$Field_Num<-c(2,1,3)
#df_var<-df_var %>% inner_join(df_var_field_id, by="Field", keep=FALSE)
#Uncomment when using JDate as a predictor
#plot(df_var$Predictions,df_var$Observed, col = df_var$JDate, pch = df_var$Field_Num,
plot(df_var$Predictions,df_var$Observed, pch = df_var$Field_Num,
main = paste("Observed vs Predicted ", className,",\n  R-squared = ",resp_fit$R2, ", MAE = ", resp_fit$MAE, sep=""), 
    xlab = paste("predicted ",className,sep=""), 
    ylab = paste("observed ",className, sep="")) 
#graphics::legend("topright", legend = unique(df_var$Field),pch = unique(df_var$Field_Num))
#Uncomment out when using JDate in the models
#graphics::legend("topleft", legend = c("June","July","Aug"), col = unique(df_var$JDate), pch = c(4,4,4))
#text(df_var$Predictions,df_var$Observed, labs=df_var$UID, cx = 0.5, dcol = "black", m = c(0, 0))
    
#plot(rf_mod_pred$predictions, testing[[className]], 
#    main = paste("Observed vs Predicted ", className,", R-squared = ",fit, sep=""), 
#    xlab = paste("predicted ",className,sep=""), 
#    ylab = paste("observed ",className, sep=""))
#plot(hexbin::hexbin(rf_mod_pred$predictions, testing[[className]]), 
#    main = paste("Observed vs Predicted ",className, sep=""), 
#    xlab = "predicted lbs acre", 
#    ylab = "observed lbs acre")
#abline(lm(rf_mod_pred$predictions~ testing[[className]]))
abline(a=0,b=1)
dev.off()
